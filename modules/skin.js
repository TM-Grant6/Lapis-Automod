...
function skinVaildate(packet, dbAccount, client, packetType) {
	...
	if (packetType === "playerList") {
		if (config.skinChecks.skinCheck1.enabled === true && !packet.skin_data.skin_id.includes(packet.skin_data.play_fab_id)) {
		...
		if (config.skinChecks.skinCheck2.enabled === true && packet.skin_data.full_skin_id != packet.skin_data.skin_id) {
		...
		if (config.skinChecks.skinCheck3.enabled === true && packet.skin_data.skin_data.width > 512 || packet.skin_data.skin_data.width < 64) {
		...
		if (config.skinChecks.skinCheck4.enabled === true && packet.skin_data.skin_data.height > 512 || packet.skin_data.skin_data.height < 64) {
		...
		if (config.skinChecks.skinCheck5.enabled === true && !packet.skin_data.skin_resource_pack.includes(packet.skin_data.play_fab_id)) {
			console.log(`[${packet.xbox_user_id}] Bad skin information [T5]`);
			if (!config.debug) client.sendCommand(`kick "${packet.xbox_user_id}" Invaild skin information sent.\nThis could be because\n- You haven't connected to PlayFab API correctly.\n- You are using classic skin (Change skin)\nTry relaunching Minecraft to fix this. (0x3f7)`, 0)
		}

		if (config.skinChecks.skinCheck6.enabled === true && packet.skin_data.play_fab_id > 16 || packet.skin_data.play_fab_id < 16) {
		...
		if (config.skinChecks.skinCheck7.enabled === true && packet.skin_data.premium === true && packet.skin_data.skin_resource_pack.includes('"default" : "geometry.n3"\n') || packet.skin_data.skin_id.includes('#')) {
		...
		if (config.skinChecks.skinCheck8.enabled === true)
			if (
		...
		if (config.skinChecks.skinCheck9.enabled === true && packet.skin_data.geometry_data_version.length < 5 || packet.skin_data.geometry_data_version.length > 6) {
		...
		if (config.skinChecks.skinCheck10.enabled === true && packet.skin_data.skin_resource_pack.includes(' "default" : "geometry.humanoid"\n')) {
		...
		if (config.skinChecks.skinCheck11.enabled === true && !packet.skin_data.skin_resource_pack.includes("default")) {
		...
		if (config.skinChecks.skinCheck1.enabled === true && !packet.skin.skin_id.includes(packet.skin.play_fab_id)) {
		...
		if (config.skinChecks.skinCheck2.enabled === true && packet.skin.full_skin_id != packet.skin.skin_id) {
		...
		if (config.skinChecks.skinCheck3.enabled === true && packet.skin.skin_data.width > 512 || packet.skin.skin_data.width < 64) {
		...
		if (config.skinChecks.skinCheck4.enabled === true && packet.skin.skin_data.height > 512 || packet.skin.skin_data.height < 64) {
		...
		if (config.skinChecks.skinCheck5.enabled === true && !packet.skin.skin_resource_pack.includes(packet.skin.play_fab_id)) {
			console.log(`[${dbAccount.xuid}] Bad skin information [T5]`);
			if (!config.debug) client.sendCommand(`kick "${dbAccount.xuid}" Invaild skin information sent.\nThis could be because\n- You are using classic skin (Change skin)\nTry relaunching Minecraft to fix this. (0x5d5)`, 0)
		}

		if (config.skinChecks.skinCheck6.enabled === true && packet.skin.play_fab_id > 16 || packet.skin.play_fab_id < 16) {
			console.log(`[${dbAccount.xuid}] Bad skin information [T6]`);
			if (!config.debug) client.sendCommand(`kick "${dbAccount.xuid}" Invaild skin information sent.\nThis could be because you are using classic skin. (Change skin) (0x5d6)`, 0)
		}

		if (config.skinChecks.skinCheck7.enabled === true && packet.skin.premium === true && packet.skin.skin_resource_pack.includes('"default" : "geometry.n3"\n') || packet.skin.skin_id.includes('#')) {
		...
		if (config.skinChecks.skinCheck10.enabled === true && packet.skin.skin_resource_pack.includes(' "default" : "geometry.humanoid"\n')) {
			console.log(`[${dbAccount.xuid}] Bad skin information [T9]`);
			if (!config.debug) client.sendCommand(`kick "${dbAccount.xuid}" Invaild skin information sent.\nThis could be becuase\nYou are wearing a corrupt skin or invisible skin.\nTry changing skins to fix this. (0x5d10)`, 0)
		}

		if (config.skinChecks.skinCheck11.enabled === true && !packet.skin.skin_resource_pack.includes("default")) {
