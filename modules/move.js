...
function moveVaildate(packet, dbAccount, client) {
	if (config.moveChecks.moveCheck1 === true && packet.position.y > 323 && !dbAccount.permission === 'operator' || !dbAccount.currentGamemode === "creative") {
		console.log(`[${dbAccount.xuid}] Bad movement (Too high) [T1]`);
		if (!config.debug) client.sendCommand(`kill ${dbAccount.currentGamertag}`);
	}

	if (config.moveChecks.moveCheck2 === true && packet.position.x >= 25000000 || packet.position.z > 25000000) {
