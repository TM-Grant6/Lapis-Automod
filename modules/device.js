...
async function deviceVaildate(packet, dbAccount, client, packetType) {
	if (packetType === "playerList") {
		...
		if (config.deviceChecks.deviceCheck2 === true && !isValidPlatformChatId(packet.platform_chat_id) && packet.build_platform === 12) {
		...
		accounts.forEach(dbAccount2 => {
			...
			if (linkedDeviceIds && Array.isArray(linkedDeviceIds)) {
				if (config.deviceChecks.deviceCheck3 === true && linkedDeviceIds.length > 4 && lastGamertag === packet.username) {
				...
				linkedDeviceIds.forEach(linkedDeviceId => {
					if (config.deviceChecks.deviceCheck4 === true && linkedDeviceId === device_id) {
		...
		if (config.deviceChecks.deviceCheck5 === true && dbAccount.deviceOs.length > 4) {
		...
		switch (device_os) {
			case "Xbox":
				if (config.deviceChecks.deviceCheck6 === true && !device_id.endsWith("=")) {
				...
				break;
			case "Android":
				if (config.deviceChecks.deviceCheck6 === true && !isUUIDv4WithoutDashes(device_id)) {
				...
				break;
			case "IOS":
				if (config.deviceChecks.deviceCheck6 === true && !isUUIDv4WithoutDashes(device_id) && /^[A-Z0-9]{32}$/.test(device_id)) {
			...
			case "Win10":
			case "Win32":
				if (config.deviceChecks.deviceCheck6 === true && !isUUIDv3(device_id)) {
				...
				break;
			case "NintendoSwitch":
				if (config.deviceChecks.deviceCheck6 === true && !isUUIDv5(device_id)) {
					console.log(`[${dbAccount.xuid}] User on Nintendo Switch with the wrong Device ID. [T6]`);
					if (!config.debug) client.sendCommand(`kick "${dbAccount.xuid}" Invalid ID. (0xc4)`, 0);
					return;
				}

				if (config.deviceChecks.deviceCheck2 === true && !isValidPlatformChatId(packet.platform_chat_id)) {
		...
		if (config.deviceChecks.deviceCheck7 === true && device_os === "Unknown" || device_os === "Dedicated" || device_os === "Linux") {
		...
		if (config.deviceChecks.deviceCheck1 === true && device_os != 'NintendoSwitch' && packet.platform_chat_id.length != 0) {
